...
static TBOOL handleInputBingo(TBYTE Ace);
...
static TBOOL handleInputBingo(TBYTE Ace)
{
    ...
    u16 value = JOY_readJoypad(JOY_1);
    	...
    	if(value & BUTTON_START)
    	{
    		...
    		VDP_clearTextLine(24);
    		VDP_clearTextLine(25);
    		VDP_drawText("See you soon!                        ",2,22);
    		NOT_PLAY=TRUE;
    		RC=TRUE;
    		SND_startPlay_2ADPCM(OBJ_MUSIC_ITEMS, sizeof(OBJ_MUSIC_ITEMS), SOUND_PCM_CH2,0);
    	}
    	...
    	if(RC && !NOT_PLAY)
    	{
    		...
    		switch(Ace)
    		{
    			case 0:{ SPR_setAnim(Characters[0].graph, 1); break;}
    			case 1:{ SPR_setAnim(Characters[1].graph, 1); break;}
    			case 2:{ SPR_setAnim(Characters[2].graph, 1); break;}
    		}

        	if(Card == Ace)
        	{

        		VDP_drawText("* * * winner! * * *                        ",2,22);
        		char msg[60]="";

        		sprintf(msg,"Your current record: %d                                  ",Gamer->total_score);
        		VDP_drawText(msg,2,23);

        		Gamer->total_score+=Gamer->total_score;
        		sprintf(msg,"Your new Record: %d                                  ",Gamer->total_score);
        		VDP_drawText(msg,2,24);
        	...
        	else
        	{
        		...
        		VDP_drawText("You lost everything!                       ",2,22);
        		char msg[60]="";
